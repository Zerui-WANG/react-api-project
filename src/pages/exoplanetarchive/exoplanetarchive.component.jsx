...
const Exoplanetarchive = () => {
    ...
    const baseUrl = "https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+" ;
    const partFrom = "FROM+";
    ...
    const handleChange = (event) => {
      ...
      setCheckedItems({...checkedItems, [event.target.value] : event.target.checked });    
       ...
       if(event.target.checked === true){
         if(localSelect!=="")
            localSelect += (","+ event.target.value);
          else
            localSelect += (event.target.value);
         if(!localFrom.includes(event.target.name)){
          ...
          if(localSelect.charAt(0)===",")
            localSelect = localSelect.substring(1);
          if(!finalUrl.includes(event.target.name.substring(0, event.target.name.length - 1)+".")){
       ...
       if(localSelect ===","){
        setSelect("");
       }else{setSelect(localSelect);
       }
        setfrom(localFrom);

        let localUrl = CORS + baseUrl + localSelect +partFrom + localFrom +partLimit+partFormatJson;
    ...
    return (
        <>
           <br/><br/>
              <CheckBoxArray name = {checkboxes[0].name.substring(0, checkboxes[0].name.length - 1)} checkBoxes={checkboxes} checkedItems={checkedItems} onChange={handleChange} />
            <br/>
